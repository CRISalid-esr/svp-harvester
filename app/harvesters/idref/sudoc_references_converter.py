...
class SudocReferencesConverter(AbesRDFReferencesConverter):
    ...
    async def convert(
        ...
        async for biblio_graph, uri in self._get_bibliographic_resource(
            ...
            issue = await self._get_issue(biblio_graph, uri, journal)
            new_ref.issue = issue
    ...
    async def _get_issue(self, pub_graph, uri, journal):
    ...
    async def _get_journal(self, biblio_graph, uri):
        ...
        for issn in biblio_graph.objects(rdflib.term.URIRef(uri), self.RDF_BIBO.issn):
            issn_value = issn.value
        logger.debug(f"Found issn: {issn_value}")
        issn_l_value = None
        ...
        return await self._get_or_create_journal(
            JournalInformations(
                ...
                titles=[title_value, title_key_value],
            )
        )
