...
class HalIdFromHalUrlRule(IdentifierInferenceRule):
    ...
    def _load_patterns():
        yml_file = os.path.join(
            os.path.abspath(os.path.dirname(__file__)), "./hal_accepted_patterns.yaml"
        )
        try:
            ...
            if "accepted_patterns" not in data:
                logger.warning(
                    f"The 'accepted_patterns' key is not present in {yml_file}"
                )
    ...
    def extract_hal_id(self, url: str | None) -> str | None:
        ...
        if not all([parsed_url.scheme, parsed_url.netloc]):
            return None
        hal_domain_keywords = ["hal.", "archives-ouvertes.fr"]
        if not any(kw in parsed_url.netloc for kw in hal_domain_keywords):
            return None
        ...
        match = re.match(rf"({'|'.join(self.accepted_patterns)})-\d+(v\d+)?", hal_id)
        if not match:
