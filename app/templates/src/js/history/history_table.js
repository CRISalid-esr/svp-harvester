...
class HistoryTable {
        ...
        return {
          ...
          columns: [
            ...
            },
            {
              width: "5%",
              title: "Refetch",
              className: "dt-refetch",
              orderable: false,
              data: null,
              defaultContent:
                "<button type='button' class='btn btn-primary dt-refetch'><i class='bi bi-arrow-clockwise'></i></button>",
            },
            { width: "10%", title: "Date" },
            ...
            { width: "15%", title: "Type de référence" },
            { width: "3%", title: "Nombre d'événements" },
            ...
            },
            {
              ...
              render: function (data, type, full) {
                return type === "display"
                  ? '<div title="' +
                      full[4] +
                      '" data-bs-toggle="tooltip" data-trigger="click">' +
                      data
                  : data;
              },
            },
  ...
  addRefetchListener() {
    this.dataTable.on("click", "td.dt-refetch", this.handleRefetch.bind(this));
  }

  async handleRefetch(event) {
    if (event.target.classList.contains("dt-refetch")) {
      sessionStorage.clear();
      const tr = event.target.closest("tr");
      const row = this.dataTable.row(tr);
      const entityName = row.data()[3];
      row
        .data()[4]
        .split(" ")
        .map((identifier) => {
          const type = identifier.split("(")[0];
          const value = identifier.split("(")[1].slice(0, -1);
          sessionStorage.setItem(type, value);
        });
      sessionStorage.setItem("name", entityName);
      window.location.href = "/admin/retrieve";
    }
  }

  buildReferenceEventsList(events, identifiers) {
    let listHtml =
      '<div><div class="list-group compact-list list-group-flush m-md-3 mx-5">';
    listHtml += `<div class="d-flex w-100 mb-2 justify-content-center align-items-center">`;
    identifiers.forEach((identifier) => {
      const type = identifier.type;
      const value = identifier.value;
      listHtml += `<span class="badge rounded-pill bg-warning text-dark mx-1">${type}(${value})</span>`;
    });
    listHtml += `</div>`;
      ...
      event.reference_events.forEach((refEvent) => {
        const title =
          refEvent.reference.titles.length > 0
            ...
            : "No title available";
        const enhancedBadge = refEvent.enhanced
          ? "<span class='badge bg-success-subtle text-secondary ms-2'>enhanced &#9733;</span>"
          : "";
        listHtml += `
  ...
  async handleCollapse(event) {
    if (event.target.classList.contains("dt-control")) {
      ...
      if (row.child.isShown()) {
        row.data()[this.subpage === "collection_history" ? 10 : 7] = SPINNER;
        row.child.hide();
        ...
        switch (this.subpage) {
          case "collection_history":
            ...
            const retrieval = await this.client.getRetrieval(row.data()[9]);
            row.data()[10] = this.buildReferenceEventsList(
              retrieval.data.harvestings,
              retrieval.data.entity.identifiers
            );
            row.child(row.data()[10]).show();
            row.child().on("click", async (event) => {
              ...
              if (link.detailShown === true) {
                ...
                const referenceDisplay = `${prettyPrintJson.toHtml(
                  currentReference
                )}`;
                if (type === "updated" || enhanced) {
                  const tabs = this.getDiffTabs(
                    referenceEventId,
                    referenceDisplay
                  );
                  link.detailShown = true;
                  link.insertAdjacentHTML("afterend", tabs);
                  let previousReferenceResponse = {
                    data: "No previous data found",
                  };
                  const { harvester, source_identifier, version } =
                    currentReference;
                  const intVersion = parseInt(version, 10);
                  if (intVersion >= 0) {
                    const previousVersion = intVersion - 1;
                    previousReferenceResponse =
                      await this.client.getReferenceByHarversterSourceIdentifierVersion(
                        harvester,
                        source_identifier,
                        previousVersion
                      );
                  ...
                  const previousReferenceDisplay = `${prettyPrintJson.toHtml(
                    previousReference
                  )}`;
                  ...
                  const deltaDisplay = `${annotatedFormatter.format(
                    delta,
                    currentReference
                  )}`;
                  document.getElementById(
                    `previous-ref-${referenceEventId}`
                  ).innerHTML = `<pre>${previousReferenceDisplay}</pre>`;
                  document.getElementById(
                    `data-diff-${referenceEventId}`
                  ).innerHTML = `<pre>${deltaDisplay}</pre>`;
                } else {
  ...
  sortReferenceContent(currentReference) {
    ...
    currentReference.document_type.sort((a, b) =>
    ...
    currentReference.identifiers.sort((a, b) =>
      a.type < b.type ? -1 : a.type > b.type ? 1 : 0
    );
    currentReference.contributions.forEach((contribution) => {
      contribution.affiliations.sort((a, b) =>
        a.source_identifier < b.source_identifier
          ? -1
          : a.source_identifier > b.source_identifier
          ? 1
          : 0
      ...
      contribution.affiliations.forEach((affiliation) => {
        ...
        affiliation.identifiers.sort((a, b) =>
          a.type < b.type
            ? -1
            : a.type > b.type
            ? 1
...
function formatIdentifiers(identifiers) {
  ...
  return formattedIdentifiers.join(" ");
}
