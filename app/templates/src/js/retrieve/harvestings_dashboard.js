...
class HarvestingDashboard {
  constructor(env, rootElement) {
    ...
    this.widgetContainerElement = rootElement.querySelector(
      "#harvesting-progress-widgets-container"
    );
    this.entityAttributesContainerElement = rootElement.querySelector(
      "#entity-attributes-card-container"
    );
  }
  ...
  updateEntityCard(entity) {
    ...
    if (entity.name?.length > 0) {
      const nameElement = stringToHTML(
        ejs.render(current_entity_card_name, { name: entity.name })
      );
    ...
    if (entity.identifiers?.length > 0) {
      for (const identifier of entity.identifiers) {
        const identifierElement = stringToHTML(
          ejs.render(current_entity_card_identifier, identifier)
        );
  ...
  updateWidgets(harvestings) {
    ...
    for (const harvesting of harvestings) {
      const widgetElement = stringToHTML(
        ejs.render(harvesting_progress_widget, harvesting)
      );
      this.widgetContainerElement.appendChild(widgetElement);
    }
    const popoverTriggerList = this.widgetContainerElement.querySelectorAll(
