...
def test_set_query(scanr_query_builder):
    ...
    assert scanr_query_builder.identifier_type.value == "idref"
    assert scanr_query_builder.identifier_value == "2556"
...
def test_build_publication_query_with_idref(scanr_query_builder):
    ...
    expected_result = {
        ...
        'query': {
            'bool': {
                'must': [{'term': {'authors.person': 'idref2556'}}]
            }
        },
        'sort': {
            'publicationDate': {'order': 'desc'}
