...
async def test_idref_convert_for_sparql_result(idref_sparql_result_for_doc):
    ...
    expected_abstract_fr = (
        "L'OBJECTIF DE CETTE ETUDE ETAIT D'ETABLIR LA FAISABILITE D'UN SYSTEME "
        "D'INSONORISATION DU BRUIT RAYONNE AU NIVEAU DES BOUCHES D'AERATION "
        "D'UN CLIMATISEUR AUTOMOBILE, A PARTIR DE L'UTILISATION DE TECHNIQUES "
        "DE CONTROLE ACTIF DU BRUIT DANS LES CONDUITS DE DISTRIBUTION D'AIR. "
        "UNE ETUDE SPECTRALE DES SIGNAUX ACOUSTIQUES PRESENTS DANS LE CONDUIT "
        "A PERMIS DE METTRE EN EVIDENCE DES ONDES ACOUSTIQUES CARACTERISTIQUES "
        "DE LA CONFIGURATION DU SYSTEME DE CLIMATISATION, AINSI QUE LA PRESENCE "
        "DE STRUCTURES TOURBILLONNAIRES ALEATOIRES DUES A LA GENERATION DE TURBULENCES "
        "DANS LES CONDUITS. UNE METHODOLOGIE EN TROIS ETAPES A ETE ADOPTEE: "
        "REALISATION D'UN SYSTEME DE CONTROLE ACTIF POUR ATTENUER LE BRUIT RAYONNE "
        "PAR LES SOURCES DETERMINISTES. POUR CE FAIRE, A PARTIR D'UNE EXPERIENCE DE LABORATOIRE "
        "NOUS AVONS COMPARE LES PERFORMANCES DE DIFFERENTS ALGORITHMES DE CONTROLE: "
        "TYPES LMS (METHODE DU TYPE DES MOINDRES CARRES) ET GMV (VARIANCE MINIMALE GENERALISEE). "
        "CETTE COMPARAISON A D'ABORD ETE EFFECTUEE AU MOYEN DE SIMULATIONS UTILISANT LES "
        "FONCTIONS DE TRANSFERTS REELLES DU CONDUIT, ET ENSUITE SUR LE SYSTEME EXPERIMENTAL "
        "LUI MEME. ANALYSE DE LA TURBULENCE PRESENTE DANS LES CONDUITS, AFIN DE REALISER UN "
        "SYSTEME PASSIF EN FORME DE GRILLE. CELUI-CI A PERMIS UN RECONDITIONNEMENT SPECTRAL DE "
        "L'ENERGIE CINETIQUE DE LA TURBULENCE, ET UNE MODIFICATION DE LA REPARTITION SPECTRALE "
        "DE LA PRESSION ACOUSTIQUE ASSOCIEE A CES SOURCES LOCALES. LES PRINCIPAUX BENEFICES "
        "DE CETTE TECHNIQUE ONT ETE, LA DIMINUTION DU NIVEAU SONORE GLOBAL, ET SURTOUT "
        "L'AMELIORATION DE LA COHERENCE ENTRE DEUX POINTS DE MESURE. CE DERNIER POINT A NOTAMMENT "
        "PERMIS D'OPTIMISER LES PERFORMANCES DU CONTROLE ACTIF. L'APPLICATION DE CE CONTROLE "
        "ACTIF ET PASSIF SUR LE SYSTEME DE CLIMATISATION, AFIN DE CONFIRMER LA VALIDITE DE "
        "LA METHODOLOGIE UTILISEE. DES RESULTATS EXPERIMENTAUX, OBTENUS A PARTIR DU SYSTEME "
        "COMPLET, ONT ETE PRESENTES ET ANALYSES"
    )
