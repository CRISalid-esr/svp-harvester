...
from app.harvesters.idref.science_plus_references_converter import SciencePlusReferencesConverter
...
async def test_science_plus_convert_for_rdf_result(
    ...
    expected_french_abstract_beginning = (
        "La Cordillère de la Côte, entre Santiago du Chili et Valparaíso a vu son accessibilité "
    )
    result = await converter_under_tests.convert(science_plus_rdf_result_for_doc)
    assert result.source_identifier == str(science_plus_rdf_result_for_doc.source_identifier)
