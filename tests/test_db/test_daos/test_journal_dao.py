...
async def test_get_journal_by_source_issn_or_eissn(async_session):
    ...
    journal_from_db = await dao.get_journal_by_source_issn_or_eissn("hal", "1234-5678")
    assert isinstance(journal_from_db, Journal)
    assert journal_from_db.issn == "1234-5678"

    journal_from_db = await dao.get_journal_by_source_issn_or_eissn(
        "hal", eissn="9876-5432"
    )
    assert isinstance(journal_from_db, Journal)
    assert journal_from_db.issn == "1234-5678"
...
async def test_get_without_issn_and_eissn(async_session):
    ...
    dao = JournalDAO(async_session)
    journal_from_db = await dao.get_journal_by_source_issn_or_eissn("hal")
